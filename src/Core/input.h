...
class Input {
    public:
        ...
        inline bool get_key(int scan_code) 
        {
            if(scan_code > 0 && scan_code < NUM_SDL_SCANCODES) {
                return KEY_PRESSED;
                }
            else
        ...
        {
            m_inputs[scan_code] = code_found;
        }

        //Called for held down key state.
        inline bool get_key_down(int scan_code)
        {
            if(scan_code > 0 && scan_code < NUM_SDL_SCANCODES) {
                return KEY_HELD;
                }
        ...
        {
            m_inputs[scan_code] = !KEY_RELEASED;
            m_down_keys[scan_code] = !KEY_RELEASED;
        }

    private:
        //Boolean array to detect if read in scan code exists within currently defined enum. Will always return 1 if scan code is within bounds. 
        bool m_inputs[NUM_SDL_SCANCODES] = {0};
        bool m_down_keys[NUM_SDL_SCANCODES] = {0};
